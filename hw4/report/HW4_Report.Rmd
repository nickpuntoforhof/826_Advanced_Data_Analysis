...
if(file_bool == TRUE){
  ...
  for(g in 1:length(gamma_range)){
    for(c in 1:length(cost_range)){
      ...
      svm.pred <- predict(svm.fold.model, testset_fold[,-16], probability = TRUE)
      svm.probs <- attr(svm.pred, "probabilities")
      pr <-prediction(svm.probs[,1], testset_fold$diabetic)
...
gamma <- 0.01
cost <- 0.01
svm.model <- svm(diabetic ~ ., data = trainset, cost = cost, gamma = gamma, type = "C-classification", probability=TRUE)
svm.pred <- predict(svm.model, testset[,-16], probability = TRUE)
svm.probs <- attr(svm.pred, "probabilities")
```


```{r, echo = FALSE}
pr <- prediction(svm.probs[,1], testset$diabetic)
...
for(c in 1:ncol(testset)){
  ...
  for(r in 1:nrow(testset)){
    ...
    perturb <- testset[r,]
    y <- perturb[1,16]
    if(c-1 %in% numeric_column_indexes){
      perturb[1,c] <- mean(testset[,c])
    } else if (c-1 %in% ordinal_column_indexes){
      perturb[1,c] <- median(testset[,c])
    } else {
      perturb[1,c] <- ifelse(perturb[1,c] == 2, 1, 2)
    }
    
    svm.pred <- predict(svm.model, perturb[,-16], probability = TRUE)
    svm.probs <- attr(svm.pred, "probabilities")
    p <- svm.probs[1,1]
    cost <- abs(y - (1+p))
    total_cost <- cost + total_cost
  }
  importance <- c(importance, total_cost/nrow(testset))
}
```

```{r, echo = FALSE}
names(importance) <- colnames(testset)
print(round(importance,3))
```
These numbers can be roughly interpreted as the average absolute error of the predictions, consider the given variable had been perturbed. Not a whole lot can be said here -- it can be observed age is clearly the most important variable.  It can also be said though, that the similar importance of most of the variables points towards why SVMs are difficult to interpret.